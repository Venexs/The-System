...
# Flag to control rectangle visibility
...
while running:
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_q:
                show_rectangle = not show_rectangle  # Toggle show_rectangle
    ...
    # Draw the rectangle and text if show_rectangle is True
    if show_rectangle:
        ...
        text_x = rectangle_x + (rectangle_width // 2) - 30
        text_y = rectangle_y + (rectangle_height // 2) - 10

        pygame.draw.rect(screen, WHITE, (rectangle_x, rectangle_y, rectangle_width, rectangle_height))
        draw_text(screen, "Things", font_bold, BLACK, text_x, text_y)
