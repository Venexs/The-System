...
SUMMON_BUTTON_CHANCE = 0.25  # 25% chance to show the summon button
...
def next():
    global mob, XP_val, rank, rew_rank
    ...
    if mob == 4:
        ...
        # Hide dungeon-related UI elements
        canvas.itemconfig(waves_txt, state="hidden")
        canvas.itemconfig(enemy, state="hidden")
        canvas.itemconfig(text1, state="hidden")
        button_1.place_forget()
        button_2.place_forget()
        canvas.itemconfig(activity1, state="hidden")
        canvas.itemconfig(activity2, state="hidden")
        canvas.itemconfig(activity3, state="hidden")
        canvas.itemconfig(activity4, state="hidden")

        # Show summon button with 25% probability
        if random.random() < SUMMON_BUTTON_CHANCE:
            canvas.itemconfigure("button_summon", state="normal")
        else:
            # Proceed to show message and quit
...
def summon_attempt():
    global waves
    boss_name = list(waves["Final"])[0]
    if random.random() < SUMMON_SUCCESS_CHANCE:
        # Successful summon
        # TODO: Add boss to shadow soldiers (e.g., update JSON or database)
        # Placeholder subprocess.Popen for summon success
        subprocess.Popen(['python', 'Anime Version/Dungeon Runs V2/arisesuccess.py', boss_name])
        # Show success message
        canvas.create_text(
            400.0,
            200.0,
            anchor="center",
            text=f"Successfully summoned {boss_name}!",
            fill="green",
        ...
        canvas.create_text(
            ...
            200.0,
            anchor="center",
            text=f"Failed to summon {boss_name}.",
            fill="red",
            font=("Montserrat Bold", 14 * -1)
        )
    # After attempt, hide the button and proceed to show message and quit
    canvas.itemconfigure("button_summon", state="hidden")
    subprocess.Popen(['python', 'Anime Version/Message/gui.py'])
    window.quit()
...
button_image_summon = PhotoImage(file=relative_to_assets("summon.png"))
button_width = button_image_summon.width()  # Get image width
button_height = button_image_summon.height()  # Get image height

# Create the button
button_summon = Button(
    canvas,
    image=button_image_summon,
    borderwidth=0,
    highlightthickness=0,
...
# Add to canvas with proper size
canvas.create_window(646, 264, window=button_summon, width=button_width, height=button_height, tags="button_summon")
...
noticetext = canvas.create_text(
    ...
    150.0,
    anchor="nw",
    text=f"Shadow Extraction Possible...",
    fill="red",
    tags="button_summon",
