
# This file was generated by the Tkinter Designer by Parth Jadhav
# https://github.com/ParthJadhav/Tkinter-Designer

from pathlib import Path
from tkinter import Tk, Canvas, Entry, Text, Button, PhotoImage
import json
import csv
import subprocess

subprocess.Popen(['python', 'sfx.py'])

OUTPUT_PATH = Path(__file__).parent
ASSETS_PATH = OUTPUT_PATH / Path(r"assets\frame0")


def relative_to_assets(path: str) -> Path:
    return ASSETS_PATH / Path(path)

window = Tk()

window.geometry("855x568")
window.configure(bg = "#FFFFFF")

window.attributes('-alpha',0.8)

canvas = Canvas(
    window,
    bg = "#FFFFFF",
    height = 568,
    width = 855,
    bd = 0,
    highlightthickness = 0,
    relief = "ridge"
)

def item_data(name,rank,category,t,r,s):
    try:
        if name!='-' and rank!='-' and category!='-':
            fout=open('Files/Temp Files/Shop Sell temp.csv', 'w', newline='')
            fw=csv.writer(fout)
            rec=[name]
            fw.writerow(rec)
            fout.close()

            subprocess.Popen(['python', 'Anime Version/Shop Sell Final/build/gui.py'])

            window.quit()
    
    except:
        print()

def name_cut(name):
    if len(name)>15:
        s=''
        for k in range(15):
            s+=name[k]
        s+='...'
        return s
    else:
        return name

canvas.place(x = 0, y = 0)
image_image_1 = PhotoImage(
    file=relative_to_assets("image_1.png"))
image_1 = canvas.create_image(
    460.0,
    324.0,
    image=image_image_1
)

image_image_2 = PhotoImage(
    file=relative_to_assets("image_2.png"))
image_2 = canvas.create_image(
    450.0,
    287.0,
    image=image_image_2
)

canvas.create_rectangle(
    23.0,
    136.0,
    832.0,
    473.0,
    fill="#2E2E2E",
    outline="")

image_image_3 = PhotoImage(
    file=relative_to_assets("image_3.png"))
image_3 = canvas.create_image(
    46.0,
    123.0,
    image=image_image_3
)

with open("Files/Inventory.json", 'r') as fson:
    data=json.load(fson)

rol=list(data.keys())

tr_n_1=tr_n_2=tr_n_3=tr_n_4=tr_n_5=tr_n_6=tr_n_7=tr_n_8='-'
tr_n_9=tr_n_10=tr_n_11=tr_n_12=tr_n_13=tr_n_14=tr_n_15=tr_n_16='-'
tr_n_17=tr_n_18=tr_n_19=tr_n_20=tr_n_21=tr_n_22=tr_n_23=tr_n_24='-'

c=-1

def get_button_image(cat):
    if cat!='-':
        if cat == 'HELM':
            return PhotoImage(file=relative_to_assets("button_2.png"))
        elif cat == 'CHESTPLATE':
            return PhotoImage(file=relative_to_assets("button_1.png"))
        elif cat == 'FIRST GAUNTLET':
            return PhotoImage(file=relative_to_assets("button_4.png"))
        elif cat == 'SECOND GAUNTLET':
            return PhotoImage(file=relative_to_assets("button_4.png"))
        elif cat == 'BOOTS':
            return PhotoImage(file=relative_to_assets("button_3.png"))
        elif cat == 'COLLAR':
            return PhotoImage(file=relative_to_assets("button_11.png"))
        elif cat == 'RING':
            return PhotoImage(file=relative_to_assets("button_10.png"))
        elif cat == 'WEAPON':
            return PhotoImage(file=relative_to_assets("button_5.png"))
        elif cat == 'MONEY':
            return PhotoImage(file=relative_to_assets("button_6.png"))
        elif cat == 'PAPER':
            return PhotoImage(file=relative_to_assets("button_7.png"))
        elif cat == 'KEY':
            return PhotoImage(file=relative_to_assets("button_12.png"))
        else:
            return PhotoImage(file=relative_to_assets("button_8.png"))
    else:
        return PhotoImage(file=relative_to_assets("button_9.png"))
    
try:
    name_1=name_cut(rol[c+1])
    tr_n_1=rol[c+1]
    qt_1=data[rol[c+1]][0]['qty']
    cat_1=data[rol[c+1]][0]['cat']
    r_1=data[rol[c+1]][0]['rank']
    d_1=data[rol[c+1]][0]['desc']
    b_1=data[rol[c+1]][0]['buff']
    db_1=data[rol[c+1]][0]['debuff']
except:
    name_1=qt_1=cat_1=r_1=d_1=b_1=db_1='-'

button_image_1=get_button_image(cat_1)

button_1 = Button(
    image=button_image_1,
    borderwidth=0,
    highlightthickness=0,
    command=lambda: item_data(tr_n_1,r_1,cat_1,d_1,b_1,db_1),
    relief="flat"
)
button_1.place(
    x=48.921630859375,
    y=147.98919677734375,
    width=68.078369140625,
    height=82.01080322265625
)

image_image_5 = PhotoImage(
    file=relative_to_assets("image_5.png"))
image_5 = canvas.create_image(
    42.0,
    189.0,
    image=image_image_5
)

canvas.create_rectangle(
    29.0,
    231.0,
    122.0,
    245.0,
    fill="#3B3B3B",
    outline="")

canvas.create_text(
    29.0,
    232.0,
    anchor="nw",
    text=tr_n_1,
    fill="#FFFFFF",
    font=("Montserrat Medium", 10 * -1)
)

canvas.create_text(
    36.0,
    215.0,
    anchor="nw",
    text=qt_1,
    fill="#FFFFFF",
    font=("Montserrat Medium", 10 * -1)
)

c+=1
try:
    name_2=name_cut(rol[c+1])
    tr_n_2=rol[c+1]
    qt_2=data[rol[c+1]][0]['qty']
    cat_2=data[rol[c+1]][0]['cat']
    r_2=data[rol[c+1]][0]['rank']
    d_2=data[rol[c+1]][0]['desc']
    b_2=data[rol[c+1]][0]['buff']
    db_2=data[rol[c+1]][0]['debuff']
except:
    name_2=qt_2=cat_2=r_2=d_2=b_2=db_2='-'

button_image_2=get_button_image(cat_2)

button_2 = Button(
    image=button_image_2,
    borderwidth=0,
    highlightthickness=0,
    command=lambda: item_data(tr_n_2,r_2,cat_2,d_2,b_2,db_2),
    relief="flat"
)
button_2.place(
    x=149.55801391601562,
    y=147.98919677734375,
    width=67.44200134277344,
    height=82.01080322265625
)

image_image_6 = PhotoImage(
    file=relative_to_assets("image_6.png"))
image_6 = canvas.create_image(
    142.0,
    189.0,
    image=image_image_6
)

canvas.create_rectangle(
    130.0,
    231.0,
    222.0,
    245.0,
    fill="#3B3B3B",
    outline="")

canvas.create_text(
    130.0,
    232.0,
    anchor="nw",
    text=name_2,
    fill="#FFFFFF",
    font=("Montserrat Medium", 10 * -1)
)

canvas.create_text(
    137.0,
    215.0,
    anchor="nw",
    text=qt_2,
    fill="#FFFFFF",
    font=("Montserrat Medium", 10 * -1)
)

c+=1
try:
    name_3=name_cut(rol[c+1])
    tr_n_3=rol[c+1]
    qt_3=data[rol[c+1]][0]['qty']
    cat_3=data[rol[c+1]][0]['cat']
    r_3=data[rol[c+1]][0]['rank']
    d_3=data[rol[c+1]][0]['desc']
    b_3=data[rol[c+1]][0]['buff']
    db_3=data[rol[c+1]][0]['debuff']
except:
    name_3=qt_3=cat_3=r_3=d_3=b_3=db_3='-'

button_image_3=get_button_image(cat_3)

button_3 = Button(
    image=button_image_3,
    borderwidth=0,
    highlightthickness=0,
    command=lambda: item_data(tr_n_3,r_3,cat_3,d_3,b_3,db_3),
    relief="flat"
)
button_3.place(
    x=250.0,
    y=148.0,
    width=68.0,
    height=82.0
)

image_image_7 = PhotoImage(
    file=relative_to_assets("image_7.png"))
image_7 = canvas.create_image(
    243.0,
    189.0,
    image=image_image_7
)

canvas.create_rectangle(
    231.0,
    231.0,
    323.0,
    245.0,
    fill="#3B3B3B",
    outline="")

canvas.create_text(
    231.0,
    232.0,
    anchor="nw",
    text=name_3,
    fill="#FFFFFF",
    font=("Montserrat Medium", 10 * -1)
)

canvas.create_text(
    237.0,
    215.0,
    anchor="nw",
    text=qt_3,
    fill="#FFFFFF",
    font=("Montserrat Medium", 10 * -1)
)

c+=1
try:
    name_4=name_cut(rol[c+1])
    tr_n_4=rol[c+1]
    qt_4=data[rol[c+1]][0]['qty']
    cat_4=data[rol[c+1]][0]['cat']
    r_4=data[rol[c+1]][0]['rank']
    d_4=data[rol[c+1]][0]['desc']
    b_4=data[rol[c+1]][0]['buff']
    db_4=data[rol[c+1]][0]['debuff']
except:
    name_4=qt_4=cat_4=r_4=d_4=b_4=db_4='-'

button_image_4=get_button_image(cat_4)

button_4 = Button(
    image=button_image_4,
    borderwidth=0,
    highlightthickness=0,
    command=lambda: item_data(tr_n_4,r_4,cat_4,d_4,b_4,db_4),
    relief="flat"
)
button_4.place(
    x=350.8307189941406,
    y=148.0,
    width=68.16929626464844,
    height=82.0
)

image_image_8 = PhotoImage(
    file=relative_to_assets("image_8.png"))
image_8 = canvas.create_image(
    344.0,
    189.0,
    image=image_image_8
)

canvas.create_rectangle(
    331.0,
    231.0,
    424.0,
    245.0,
    fill="#3B3B3B",
    outline="")

canvas.create_text(
    331.0,
    232.0,
    anchor="nw",
    text=name_4,
    fill="#FFFFFF",
    font=("Montserrat Medium", 10 * -1)
)

canvas.create_text(
    338.0,
    215.0,
    anchor="nw",
    text=qt_4,
    fill="#FFFFFF",
    font=("Montserrat Medium", 10 * -1)
)

c+=1
try:
    name_5=name_cut(rol[c+1])
    tr_n_5=rol[c+1]
    qt_5=data[rol[c+1]][0]['qty']
    cat_5=data[rol[c+1]][0]['cat']
    r_5=data[rol[c+1]][0]['rank']
    d_5=data[rol[c+1]][0]['desc']
    b_5=data[rol[c+1]][0]['buff']
    db_5=data[rol[c+1]][0]['debuff']
except:
    name_5=qt_5=cat_5=r_5=d_5=b_5=db_5='-'

button_image_5=get_button_image(cat_5)

button_5 = Button(
    image=button_image_5,
    borderwidth=0,
    highlightthickness=0,
    command=lambda: item_data(tr_n_5,r_5,cat_5,d_5,b_5,db_5),
    relief="flat"
)
button_5.place(
    x=451.467041015625,
    y=148.0,
    width=67.53294372558594,
    height=82.0
)

image_image_9 = PhotoImage(
    file=relative_to_assets("image_9.png"))
image_9 = canvas.create_image(
    444.0,
    189.0,
    image=image_image_9
)

canvas.create_rectangle(
    432.0,
    231.0,
    524.0,
    245.0,
    fill="#3B3B3B",
    outline="")

canvas.create_text(
    432.0,
    232.0,
    anchor="nw",
    text=name_5,
    fill="#FFFFFF",
    font=("Montserrat Medium", 10 * -1)
)

canvas.create_text(
    439.0,
    215.0,
    anchor="nw",
    text=qt_5,
    fill="#FFFFFF",
    font=("Montserrat Medium", 10 * -1)
)

c+=1
try:
    name_6=name_cut(rol[c+1])
    tr_n_6=rol[c+1]
    qt_6=data[rol[c+1]][0]['qty']
    cat_6=data[rol[c+1]][0]['cat']
    r_6=data[rol[c+1]][0]['rank']
    d_6=data[rol[c+1]][0]['desc']
    b_6=data[rol[c+1]][0]['buff']
    db_6=data[rol[c+1]][0]['debuff']
except:
    name_6=qt_6=cat_6=r_6=d_6=b_6=db_6='-'

button_image_6=get_button_image(cat_6)

button_6 = Button(
    image=button_image_6,
    borderwidth=0,
    highlightthickness=0,
    command=lambda: item_data(tr_n_6,r_6,cat_6,d_6,b_6,db_6),
    relief="flat"
)
button_6.place(
    x=552.0,
    y=148.0,
    width=68.0,
    height=82.0
)

image_image_10 = PhotoImage(
    file=relative_to_assets("image_10.png"))
image_10 = canvas.create_image(
    545.0,
    189.0,
    image=image_image_10
)

canvas.create_rectangle(
    532.0,
    231.0,
    625.0,
    245.0,
    fill="#3B3B3B",
    outline="")

canvas.create_text(
    532.0,
    232.0,
    anchor="nw",
    text=name_6,
    fill="#FFFFFF",
    font=("Montserrat Medium", 10 * -1)
)

canvas.create_text(
    539.0,
    215.0,
    anchor="nw",
    text=qt_6,
    fill="#FFFFFF",
    font=("Montserrat Medium", 10 * -1)
)

c+=1
try:
    name_7=name_cut(rol[c+1])
    tr_n_7=rol[c+1]
    qt_7=data[rol[c+1]][0]['qty']
    cat_7=data[rol[c+1]][0]['cat']
    r_7=data[rol[c+1]][0]['rank']
    d_7=data[rol[c+1]][0]['desc']
    b_7=data[rol[c+1]][0]['buff']
    db_7=data[rol[c+1]][0]['debuff']
except:
    name_7=qt_7=cat_7=r_7=d_7=b_7=db_7='-'

button_image_7=get_button_image(cat_7)

button_7 = Button(
    image=button_image_7,
    borderwidth=0,
    highlightthickness=0,
    command=lambda: item_data(tr_n_7,r_7,cat_7,d_7,b_7,db_7),
    relief="flat"
)
button_7.place(
    x=652.7398071289062,
    y=147.98919677734375,
    width=67.26017761230469,
    height=82.01080322265625
)

image_image_11 = PhotoImage(
    file=relative_to_assets("image_11.png"))
image_11 = canvas.create_image(
    646.0,
    189.0,
    image=image_image_11
)

canvas.create_rectangle(
    633.0,
    231.0,
    726.0,
    245.0,
    fill="#3B3B3B",
    outline="")

canvas.create_text(
    633.0,
    232.0,
    anchor="nw",
    text=name_7,
    fill="#FFFFFF",
    font=("Montserrat Medium", 10 * -1)
)

canvas.create_text(
    640.0,
    215.0,
    anchor="nw",
    text=qt_7,
    fill="#FFFFFF",
    font=("Montserrat Medium", 10 * -1)
)

c+=1
try:
    name_8=name_cut(rol[c+1])
    tr_n_8=rol[c+1]
    qt_8=data[rol[c+1]][0]['qty']
    cat_8=data[rol[c+1]][0]['cat']
    r_8=data[rol[c+1]][0]['rank']
    d_8=data[rol[c+1]][0]['desc']
    b_8=data[rol[c+1]][0]['buff']
    db_8=data[rol[c+1]][0]['debuff']
except:
    name_8=qt_8=cat_8=r_8=d_8=b_8=db_8='-'

button_image_8=get_button_image(cat_8)

button_8 = Button(
    image=button_image_8,
    borderwidth=0,
    highlightthickness=0,
    command=lambda: item_data(tr_n_8,r_8,cat_8,d_8,b_8,db_8),
    relief="flat"
)
button_8.place(
    x=754.0,
    y=147.98919677734375,
    width=67.0,
    height=82.01080322265625
)

image_image_12 = PhotoImage(
    file=relative_to_assets("image_12.png"))
image_12 = canvas.create_image(
    746.0,
    189.0,
    image=image_image_12
)

canvas.create_rectangle(
    734.0,
    231.0,
    826.0,
    245.0,
    fill="#3B3B3B",
    outline="")

canvas.create_text(
    734.0,
    232.0,
    anchor="nw",
    text=name_8,
    fill="#FFFFFF",
    font=("Montserrat Medium", 10 * -1)
)

canvas.create_text(
    741.0,
    215.0,
    anchor="nw",
    text=qt_8,
    fill="#FFFFFF",
    font=("Montserrat Medium", 10 * -1)
)


# ! =====================================================================================

c+=1
try:
    name_9=name_cut(rol[c+1])
    tr_n_9=rol[c+1]
    qt_9=data[rol[c+1]][0]['qty']
    cat_9=data[rol[c+1]][0]['cat']
    r_9=data[rol[c+1]][0]['rank']
    d_9=data[rol[c+1]][0]['desc']
    b_9=data[rol[c+1]][0]['buff']
    db_9=data[rol[c+1]][0]['debuff']
except:
    name_9=qt_9=cat_9=r_9=d_9=b_9=db_9='-'

button_image_9=get_button_image(cat_9)

button_9 = Button(
    image=button_image_9,
    borderwidth=0,
    highlightthickness=0,
    command=lambda:item_data(tr_n_9,r_9,cat_9,d_9,b_9,db_9),
    relief="flat"
)
button_9.place(
    x=50.0,
    y=257.0,
    width=68.0,
    height=82.0
)

image_image_13 = PhotoImage(
    file=relative_to_assets("image_13.png"))
image_13 = canvas.create_image(
    43.0,
    298.0,
    image=image_image_13
)

canvas.create_rectangle(
    29.0,
    339.0,
    122.0,
    353.0,
    fill="#3B3B3B",
    outline="")

canvas.create_text(
    29.0,
    341.0,
    anchor="nw",
    text=name_9,
    fill="#FFFFFF",
    font=("Montserrat Medium", 10 * -1)
)

canvas.create_text(
    36.0,
    325.0,
    anchor="nw",
    text=qt_9,
    fill="#FFFFFF",
    font=("Montserrat Medium", 10 * -1)
)

c+=1
try:
    name_10=name_cut(rol[c+1])
    tr_n_10=rol[c+1]
    qt_10=data[rol[c+1]][0]['qty']
    cat_10=data[rol[c+1]][0]['cat']
    r_10=data[rol[c+1]][0]['rank']
    d_10=data[rol[c+1]][0]['desc']
    b_10=data[rol[c+1]][0]['buff']
    db_10=data[rol[c+1]][0]['debuff']
except:
    name_10=qt_10=cat_10=r_10=d_10=b_10=db_10='-'

button_image_10=get_button_image(cat_10)

button_10 = Button(
    image=button_image_10,
    borderwidth=0,
    highlightthickness=0,
    command=lambda:item_data(tr_n_10,r_10,cat_10,d_10,b_10,db_10),
    relief="flat"
)
button_10.place(
    x=150.0,
    y=257.0,
    width=67.0,
    height=82.0
)

image_image_14 = PhotoImage(
    file=relative_to_assets("image_14.png"))
image_14 = canvas.create_image(
    142.0,
    298.0,
    image=image_image_14
)

canvas.create_rectangle(
    130.0,
    339.0,
    222.0,
    353.0,
    fill="#3B3B3B",
    outline="")

canvas.create_text(
    130.0,
    341.0,
    anchor="nw",
    text=name_10,
    fill="#FFFFFF",
    font=("Montserrat Medium", 10 * -1)
)

canvas.create_text(
    137.0,
    325.0,
    anchor="nw",
    text=qt_10,
    fill="#FFFFFF",
    font=("Montserrat Medium", 10 * -1)
)

c+=1
try:
    name_11=name_cut(rol[c+1])
    tr_n_11=rol[c+1]
    qt_11=data[rol[c+1]][0]['qty']
    cat_11=data[rol[c+1]][0]['cat']
    r_11=data[rol[c+1]][0]['rank']
    d_11=data[rol[c+1]][0]['desc']
    b_11=data[rol[c+1]][0]['buff']
    db_11=data[rol[c+1]][0]['debuff']
except:
    name_11=qt_11=cat_11=r_11=d_11=b_11=db_11='-'

button_image_11=get_button_image(cat_11)

button_11 = Button(
    image=button_image_11,
    borderwidth=0,
    highlightthickness=0,
    command=lambda:item_data(tr_n_11,r_11,cat_11,d_11,b_11,db_11),
    relief="flat"
)
button_11.place(
    x=250.0,
    y=257.0,
    width=68.0,
    height=82.0
)

image_image_15 = PhotoImage(
    file=relative_to_assets("image_15.png"))
image_15 = canvas.create_image(
    243.0,
    298.0,
    image=image_image_15
)

canvas.create_rectangle(
    231.0,
    339.0,
    323.0,
    353.0,
    fill="#3B3B3B",
    outline="")

canvas.create_text(
    231.0,
    341.0,
    anchor="nw",
    text=name_11,
    fill="#FFFFFF",
    font=("Montserrat Medium", 10 * -1)
)

canvas.create_text(
    237.0,
    325.0,
    anchor="nw",
    text=qt_11,
    fill="#FFFFFF",
    font=("Montserrat Medium", 10 * -1)
)

c+=1
try:
    name_12=name_cut(rol[c+1])
    tr_n_12=rol[c+1]
    qt_12=data[rol[c+1]][0]['qty']
    cat_12=data[rol[c+1]][0]['cat']
    r_12=data[rol[c+1]][0]['rank']
    d_12=data[rol[c+1]][0]['desc']
    b_12=data[rol[c+1]][0]['buff']
    db_12=data[rol[c+1]][0]['debuff']
except:
    name_12=qt_12=cat_12=r_12=d_12=b_12=db_12='-'

button_image_12=get_button_image(cat_12)

button_12 = Button(
    image=button_image_12,
    borderwidth=0,
    highlightthickness=0,
    command=lambda: item_data(tr_n_12,r_12,cat_12,d_12,b_12,db_12),
    relief="flat"
)
button_12.place(
    x=351.0,
    y=257.0,
    width=67.0,
    height=82.0
)

image_image_16 = PhotoImage(
    file=relative_to_assets("image_16.png"))
image_16 = canvas.create_image(
    344.0,
    298.0,
    image=image_image_16
)

canvas.create_rectangle(
    331.0,
    339.0,
    424.0,
    353.0,
    fill="#3B3B3B",
    outline="")

canvas.create_text(
    331.0,
    341.0,
    anchor="nw",
    text=name_12,
    fill="#FFFFFF",
    font=("Montserrat Medium", 10 * -1)
)

canvas.create_text(
    338.0,
    325.0,
    anchor="nw",
    text=qt_12,
    fill="#FFFFFF",
    font=("Montserrat Medium", 10 * -1)
)

c+=1
try:
    name_13=name_cut(rol[c+1])
    tr_n_13=rol[c+1]
    qt_13=data[rol[c+1]][0]['qty']
    cat_13=data[rol[c+1]][0]['cat']
    r_13=data[rol[c+1]][0]['rank']
    d_13=data[rol[c+1]][0]['desc']
    b_13=data[rol[c+1]][0]['buff']
    db_13=data[rol[c+1]][0]['debuff']
except:
    name_13=qt_13=cat_13=r_13=d_13=b_13=db_13='-'

button_image_13=get_button_image(cat_13)

button_13 = Button(
    image=button_image_13,
    borderwidth=0,
    highlightthickness=0,
    command=lambda: item_data(tr_n_13,r_13,cat_13,d_13,b_13,db_13),
    relief="flat"
)
button_13.place(
    x=452.0,
    y=257.0,
    width=67.0,
    height=82.0
)

image_image_17 = PhotoImage(
    file=relative_to_assets("image_17.png"))
image_17 = canvas.create_image(
    444.0,
    298.0,
    image=image_image_17
)

canvas.create_rectangle(
    432.0,
    339.0,
    524.0,
    353.0,
    fill="#3B3B3B",
    outline="")

canvas.create_text(
    432.0,
    341.0,
    anchor="nw",
    text=name_13,
    fill="#FFFFFF",
    font=("Montserrat Medium", 10 * -1)
)

canvas.create_text(
    439.0,
    325.0,
    anchor="nw",
    text=qt_13,
    fill="#FFFFFF",
    font=("Montserrat Medium", 10 * -1)
)

c+=1
try:
    name_14=name_cut(rol[c+1])
    tr_n_14=rol[c+1]
    qt_14=data[rol[c+1]][0]['qty']
    cat_14=data[rol[c+1]][0]['cat']
    r_14=data[rol[c+1]][0]['rank']
    d_14=data[rol[c+1]][0]['desc']
    b_14=data[rol[c+1]][0]['buff']
    db_14=data[rol[c+1]][0]['debuff']
except:
    name_14=qt_14=cat_14=r_14=d_14=b_14=db_14='-'

button_image_14=get_button_image(cat_14)

button_14 = Button(
    image=button_image_14,
    borderwidth=0,
    highlightthickness=0,
    command=lambda: item_data(tr_n_14,r_14,cat_14,d_14,b_14,db_14),
    relief="flat"
)
button_14.place(
    x=553.0,
    y=257.0,
    width=67.0,
    height=82.0
)

image_image_18 = PhotoImage(
    file=relative_to_assets("image_18.png"))
image_18 = canvas.create_image(
    545.0,
    298.0,
    image=image_image_18
)

canvas.create_rectangle(
    532.0,
    339.0,
    625.0,
    353.0,
    fill="#3B3B3B",
    outline="")

canvas.create_text(
    532.0,
    341.0,
    anchor="nw",
    text=name_14,
    fill="#FFFFFF",
    font=("Montserrat Medium", 10 * -1)
)

canvas.create_text(
    539.0,
    325.0,
    anchor="nw",
    text=qt_14,
    fill="#FFFFFF",
    font=("Montserrat Medium", 10 * -1)
)

c+=1
try:
    name_15=name_cut(rol[c+1])
    tr_n_15=rol[c+1]
    qt_15=data[rol[c+1]][0]['qty']
    cat_15=data[rol[c+1]][0]['cat']
    r_15=data[rol[c+1]][0]['rank']
    d_15=data[rol[c+1]][0]['desc']
    b_15=data[rol[c+1]][0]['buff']
    db_15=data[rol[c+1]][0]['debuff']
except:
    name_15=qt_15=cat_15=r_15=d_15=b_15=db_15='-'

button_image_15=get_button_image(cat_15)

button_15 = Button(
    image=button_image_15,
    borderwidth=0,
    highlightthickness=0,
    command=lambda: item_data(tr_n_15,r_15,cat_15,d_15,b_15,db_15),
    relief="flat"
)
button_15.place(
    x=653.0,
    y=257.0,
    width=67.0,
    height=82.0
)

image_image_19 = PhotoImage(
    file=relative_to_assets("image_19.png"))
image_19 = canvas.create_image(
    646.0,
    298.0,
    image=image_image_19
)

canvas.create_rectangle(
    633.0,
    339.0,
    726.0,
    353.0,
    fill="#3B3B3B",
    outline="")

canvas.create_text(
    633.0,
    341.0,
    anchor="nw",
    text=name_15,
    fill="#FFFFFF",
    font=("Montserrat Medium", 10 * -1)
)

canvas.create_text(
    640.0,
    325.0,
    anchor="nw",
    text=qt_15,
    fill="#FFFFFF",
    font=("Montserrat Medium", 10 * -1)
)

c+=1
try:
    name_16=name_cut(rol[c+1])
    tr_n_16=rol[c+1]
    qt_16=data[rol[c+1]][0]['qty']
    cat_16=data[rol[c+1]][0]['cat']
    r_16=data[rol[c+1]][0]['rank']
    d_16=data[rol[c+1]][0]['desc']
    b_16=data[rol[c+1]][0]['buff']
    db_16=data[rol[c+1]][0]['debuff']
except:
    name_16=qt_16=cat_16=r_16=d_16=b_16=db_16='-'

button_image_16=get_button_image(cat_16)

button_16 = Button(
    image=button_image_16,
    borderwidth=0,
    highlightthickness=0,
    command=lambda:item_data(tr_n_16,r_16,cat_16,d_16,b_16,db_16),
    relief="flat"
)
button_16.place(
    x=754.0,
    y=257.0,
    width=67.0,
    height=82.0
)

image_image_20 = PhotoImage(
    file=relative_to_assets("image_20.png"))
image_20 = canvas.create_image(
    746.0,
    298.0,
    image=image_image_20
)

canvas.create_rectangle(
    734.0,
    339.0,
    826.0,
    353.0,
    fill="#3B3B3B",
    outline="")

canvas.create_text(
    734.0,
    341.0,
    anchor="nw",
    text=name_16,
    fill="#FFFFFF",
    font=("Montserrat Medium", 10 * -1)
)

canvas.create_text(
    741.0,
    325.0,
    anchor="nw",
    text=qt_16,
    fill="#FFFFFF",
    font=("Montserrat Medium", 10 * -1)
)

# ! ================================================================================================

c+=1
try:
    name_17=name_cut(rol[c+1])
    tr_n_17=rol[c+1]
    qt_17=data[rol[c+1]][0]['qty']
    cat_17=data[rol[c+1]][0]['cat']
    r_17=data[rol[c+1]][0]['rank']
    d_17=data[rol[c+1]][0]['desc']
    b_17=data[rol[c+1]][0]['buff']
    db_17=data[rol[c+1]][0]['debuff']
except:
    name_17=qt_17=cat_17=r_17=d_17=b_17=db_17='-'

button_image_17=get_button_image(cat_17)

button_17 = Button(
    image=button_image_17,
    borderwidth=0,
    highlightthickness=0,
    command=lambda: item_data(tr_n_17,r_17,cat_17,d_17,b_17,db_17),
    relief="flat"
)
button_17.place(
    x=48.0,
    y=364.0,
    width=69.0,
    height=82.0
)

image_image_21 = PhotoImage(
    file=relative_to_assets("image_21.png"))
image_21 = canvas.create_image(
    42.0,
    405.0,
    image=image_image_21
)

canvas.create_rectangle(
    29.0,
    447.0,
    122.0,
    461.0,
    fill="#3B3B3B",
    outline="")

canvas.create_text(
    29.0,
    448.0,
    anchor="nw",
    text=name_17,
    fill="#FFFFFF",
    font=("Montserrat Medium", 10 * -1)
)

canvas.create_text(
    36.0,
    433.0,
    anchor="nw",
    text=qt_17,
    fill="#FFFFFF",
    font=("Montserrat Medium", 10 * -1)
)

c+=1
try:
    name_18=name_cut(rol[c+1])
    tr_n_18=rol[c+1]
    qt_18=data[rol[c+1]][0]['qty']
    cat_18=data[rol[c+1]][0]['cat']
    r_18=data[rol[c+1]][0]['rank']
    d_18=data[rol[c+1]][0]['desc']
    b_18=data[rol[c+1]][0]['buff']
    db_18=data[rol[c+1]][0]['debuff']
except:
    name_18=qt_18=cat_18=r_18=d_18=b_18=db_18='-'

button_image_18=get_button_image(cat_18)

button_18 = Button(
    image=button_image_18,
    borderwidth=0,
    highlightthickness=0,
    command=lambda: item_data(tr_n_18,r_18,cat_18,d_18,b_18,db_18),
    relief="flat"
)
button_18.place(
    x=150.0,
    y=364.0,
    width=67.0,
    height=82.0
)

image_image_22 = PhotoImage(
    file=relative_to_assets("image_22.png"))
image_22 = canvas.create_image(
    142.0,
    405.0,
    image=image_image_22
)

canvas.create_rectangle(
    130.0,
    447.0,
    222.0,
    461.0,
    fill="#3B3B3B",
    outline="")

canvas.create_text(
    130.0,
    448.0,
    anchor="nw",
    text=name_18,
    fill="#FFFFFF",
    font=("Montserrat Medium", 10 * -1)
)

canvas.create_text(
    137.0,
    433.0,
    anchor="nw",
    text=qt_18,
    fill="#FFFFFF",
    font=("Montserrat Medium", 10 * -1)
)

c+=1
try:
    name_19=name_cut(rol[c+1])
    tr_n_19=rol[c+1]
    qt_19=data[rol[c+1]][0]['qty']
    cat_19=data[rol[c+1]][0]['cat']
    r_19=data[rol[c+1]][0]['rank']
    d_19=data[rol[c+1]][0]['desc']
    b_19=data[rol[c+1]][0]['buff']
    db_19=data[rol[c+1]][0]['debuff']
except:
    name_19=qt_19=cat_19=r_19=d_19=b_19=db_19='-'

button_image_19=get_button_image(cat_19)

button_19 = Button(
    image=button_image_19,
    borderwidth=0,
    highlightthickness=0,
    command=lambda: item_data(tr_n_19,r_19,cat_19,d_19,b_19,db_19),
    relief="flat"
)
button_19.place(
    x=250.0,
    y=364.0,
    width=68.0,
    height=82.0
)

image_image_23 = PhotoImage(
    file=relative_to_assets("image_23.png"))
image_23 = canvas.create_image(
    243.0,
    405.0,
    image=image_image_23
)

canvas.create_rectangle(
    231.0,
    447.0,
    323.0,
    461.0,
    fill="#3B3B3B",
    outline="")

canvas.create_text(
    231.0,
    448.0,
    anchor="nw",
    text=name_19,
    fill="#FFFFFF",
    font=("Montserrat Medium", 10 * -1)
)

canvas.create_text(
    237.0,
    433.0,
    anchor="nw",
    text=qt_19,
    fill="#FFFFFF",
    font=("Montserrat Medium", 10 * -1)
)

c+=1
try:
    name_20=name_cut(rol[c+1])
    tr_n_20=rol[c+1]
    qt_20=data[rol[c+1]][0]['qty']
    cat_20=data[rol[c+1]][0]['cat']
    r_20=data[rol[c+1]][0]['rank']
    d_20=data[rol[c+1]][0]['desc']
    b_20=data[rol[c+1]][0]['buff']
    db_20=data[rol[c+1]][0]['debuff']
except:
    name_20=qt_20=cat_20=r_20=d_20=b_20=db_20='-'

button_image_20=get_button_image(cat_20)

button_20 = Button(
    image=button_image_20,
    borderwidth=0,
    highlightthickness=0,
    command=lambda: item_data(tr_n_20,r_20,cat_20,d_20,b_20,db_20),
    relief="flat"
)
button_20.place(
    x=350.0,
    y=364.0,
    width=68.0,
    height=82.0
)

image_image_24 = PhotoImage(
    file=relative_to_assets("image_24.png"))
image_24 = canvas.create_image(
    344.0,
    405.0,
    image=image_image_24
)

canvas.create_rectangle(
    331.0,
    447.0,
    424.0,
    461.0,
    fill="#3B3B3B",
    outline="")

canvas.create_text(
    331.0,
    448.0,
    anchor="nw",
    text=name_20,
    fill="#FFFFFF",
    font=("Montserrat Medium", 10 * -1)
)

canvas.create_text(
    338.0,
    433.0,
    anchor="nw",
    text=qt_20,
    fill="#FFFFFF",
    font=("Montserrat Medium", 10 * -1)
)

c+=1
try:
    name_21=name_cut(rol[c+1])
    tr_n_21=rol[c+1]
    qt_21=data[rol[c+1]][0]['qty']
    cat_21=data[rol[c+1]][0]['cat']
    r_21=data[rol[c+1]][0]['rank']
    d_21=data[rol[c+1]][0]['desc']
    b_21=data[rol[c+1]][0]['buff']
    db_21=data[rol[c+1]][0]['debuff']
except:
    name_21=qt_21=cat_21=r_21=d_21=b_21=db_21='-'

button_image_21=get_button_image(cat_21)

button_21 = Button(
    image=button_image_21,
    borderwidth=0,
    highlightthickness=0,
    command=lambda: item_data(tr_n_21,r_21,cat_21,d_21,b_21,db_21),
    relief="flat"
)
button_21.place(
    x=452.0,
    y=364.0,
    width=67.0,
    height=82.0
)

image_image_25 = PhotoImage(
    file=relative_to_assets("image_25.png"))
image_25 = canvas.create_image(
    444.0,
    405.0,
    image=image_image_25
)

canvas.create_rectangle(
    432.0,
    447.0,
    524.0,
    461.0,
    fill="#3B3B3B",
    outline="")

canvas.create_text(
    432.0,
    448.0,
    anchor="nw",
    text=name_21,
    fill="#FFFFFF",
    font=("Montserrat Medium", 10 * -1)
)

canvas.create_text(
    439.0,
    433.0,
    anchor="nw",
    text=qt_21,
    fill="#FFFFFF",
    font=("Montserrat Medium", 10 * -1)
)

c+=1
try:
    name_22=name_cut(rol[c+1])
    tr_n_22=rol[c+1]
    qt_22=data[rol[c+1]][0]['qty']
    cat_22=data[rol[c+1]][0]['cat']
    r_22=data[rol[c+1]][0]['rank']
    d_22=data[rol[c+1]][0]['desc']
    b_22=data[rol[c+1]][0]['buff']
    db_22=data[rol[c+1]][0]['debuff']
except:
    name_22=qt_22=cat_22=r_22=d_22=b_22=db_22='-'

button_image_22=get_button_image(cat_22)

button_22 = Button(
    image=button_image_22,
    borderwidth=0,
    highlightthickness=0,
    command=lambda: item_data(tr_n_22,r_22,cat_22,d_22,b_22,db_22),
    relief="flat"
)
button_22.place(
    x=553.0,
    y=364.0,
    width=67.0,
    height=82.0
)

image_image_26 = PhotoImage(
    file=relative_to_assets("image_26.png"))
image_26 = canvas.create_image(
    545.0,
    405.0,
    image=image_image_26
)

canvas.create_rectangle(
    532.0,
    447.0,
    625.0,
    461.0,
    fill="#3B3B3B",
    outline="")

canvas.create_text(
    532.0,
    448.0,
    anchor="nw",
    text=name_22,
    fill="#FFFFFF",
    font=("Montserrat Medium", 10 * -1)
)

canvas.create_text(
    539.0,
    433.0,
    anchor="nw",
    text=qt_22,
    fill="#FFFFFF",
    font=("Montserrat Medium", 10 * -1)
)

c+=1
try:
    name_23=name_cut(rol[c+1])
    tr_n_23=rol[c+1]
    qt_23=data[rol[c+1]][0]['qty']
    cat_23=data[rol[c+1]][0]['cat']
    r_23=data[rol[c+1]][0]['rank']
    d_23=data[rol[c+1]][0]['desc']
    b_23=data[rol[c+1]][0]['buff']
    db_23=data[rol[c+1]][0]['debuff']
except:
    name_23=qt_23=cat_23=r_23=d_23=b_23=db_23='-'

button_image_23=get_button_image(cat_23)

button_23 = Button(
    image=button_image_23,
    borderwidth=0,
    highlightthickness=0,
    command=lambda: item_data(tr_n_23,r_23,cat_23,d_23,b_23,db_23),
    relief="flat"
)
button_23.place(
    x=653.0,
    y=364.0,
    width=67.0,
    height=82.0
)

image_image_27 = PhotoImage(
    file=relative_to_assets("image_27.png"))
image_27 = canvas.create_image(
    646.0,
    405.0,
    image=image_image_27
)

canvas.create_rectangle(
    633.0,
    447.0,
    726.0,
    461.0,
    fill="#3B3B3B",
    outline="")

canvas.create_text(
    633.0,
    448.0,
    anchor="nw",
    text=name_23,
    fill="#FFFFFF",
    font=("Montserrat Medium", 10 * -1)
)

canvas.create_text(
    640.0,
    433.0,
    anchor="nw",
    text=qt_23,
    fill="#FFFFFF",
    font=("Montserrat Medium", 10 * -1)
)

c+=1
try:
    name_24=name_cut(rol[c+1])
    tr_n_24=rol[c+1]
    qt_24=data[rol[c+1]][0]['qty']
    cat_24=data[rol[c+1]][0]['cat']
    r_24=data[rol[c+1]][0]['rank']
    d_24=data[rol[c+1]][0]['desc']
    b_24=data[rol[c+1]][0]['buff']
    db_24=data[rol[c+1]][0]['debuff']
except:
    name_24=qt_24=cat_24=r_24=d_24=b_24=db_24='-'

button_image_24=get_button_image(cat_24)

button_24 = Button(
    image=button_image_24,
    borderwidth=0,
    highlightthickness=0,
    command=lambda: item_data(tr_n_24,r_24,cat_24,d_24,b_24,db_24),
    relief="flat"
)
button_24.place(
    x=754.0,
    y=364.0,
    width=67.0,
    height=82.0
)

image_image_28 = PhotoImage(
    file=relative_to_assets("image_28.png"))
image_28 = canvas.create_image(
    746.0,
    405.0,
    image=image_image_28
)

canvas.create_rectangle(
    734.0,
    447.0,
    826.0,
    461.0,
    fill="#3B3B3B",
    outline="")

canvas.create_text(
    734.0,
    448.0,
    anchor="nw",
    text=name_24,
    fill="#FFFFFF",
    font=("Montserrat Medium", 10 * -1)
)

canvas.create_text(
    741.0,
    433.0,
    anchor="nw",
    text=qt_24,
    fill="#FFFFFF",
    font=("Montserrat Medium", 10 * -1)
)

window.resizable(False, False)
window.mainloop()
